...
void gem_qc(TString filename) {
    ...
    while ( input>>delay>>currentString>>pvoltageset>>pvoltagemon ) {

      // Sets NaN readings to overflow value
      if ( (currentString=="NaN") || (currentString=="NAN") )
      	pcurrent = 999;
      else pcurrent = atof(currentString.c_str());

      // Counts distinct sparks (current spikes)
      if ( (abs(pcurrent)) > sparkThreshold)
	// Is true if voltage passes threshold 
	currentHigh = 1;      
      else if (currentHigh) {
    ...
    TH2F *currentVtime = new TH2F("currentVtime","",nbinTime,timeMin,timeMax,nbinCurr,currentMin,currentMax);
    ...
    TH2F *voltageVtime = new TH2F("voltageVtime","",nbinTime,timeMin,timeMax,nbinVolt,voltageMin,voltageMax);
    ...
    currentVtime->SetMarkerColorAlpha(0, 0);
    gPad->SetGrid();    
